...
- [ ] 1.1 Initialize Next.js 15 app with TypeScript, Tailwind CSS, shadcn/ui, Geist font, and App Router
...
- [ ] 4.1 Build signed-in upload page UI: drag-and-drop zone + file picker, optional title input, manual Upload button, and progress bar
...
- [ ] 4.6 Show post-upload success panel on same page: share link, copy button, Preview link to `/t/[slug]`, and Upload another
...
- [ ] 5.1 Build anonymous upload UI: drag-and-drop zone + file picker, manual Upload button, no title field (filename used as title), clear 10-minute expiry messaging
...
- [ ] 6.3 Build custom audio player: play/pause, seek scrubber, duration display, play-button spinner while stream loads, stream URL refresh (no volume, no keyboard shortcuts)
- [ ] 6.4 Add Open Graph meta tags (title, description "Listen on throw.it", static og:image) to playback page
- [ ] 6.5 Build generic 404 page ("Track not found") for invalid, deleted, and expired slugs
...
- [ ] 7.1 Build library page at `/library` as a table: title, duration, date, Copy link / Rename / Delete actions
...
- [ ] 7.4 Implement rename via dialog UI and API route with ownership check
- [ ] 7.5 Implement delete via confirm dialog, API route with ownership check, R2 object removal, slug retirement, and quota update
...
- [ ] 9.2 Add root app shell: nav on every page (wordmark, library link for auth'd users, theme toggle, sign in/out); no footer
...
- [ ] 9.5 Build `/sign-in` with inline "Check your email" state after magic link request
